...
else if(!empty($_REQUEST['Approve']))
{
	...
	if($rep_link_to_approved != -1) {
		...
		$shortUrl = getSmallLink("http://databib.lib.purdue.edu/viewapprovedbyrecordid.php?record=$insertId");
	
		post_tweet("A new data repository, \"$rep_title\" was added to Databib, $shortUrl. Follow @databib for future updates!");
	...
	$sql = "SELECT * FROM notapproved where reviewed='n'";
	$result = mysql_query($sql) or die(mysql_error());
	$row = mysql_fetch_array($result);
